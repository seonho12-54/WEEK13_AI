...
이번 실험은 optimizer를 `Adam`으로 고정하고, BatchNorm/Dropout 적용 여부, Dropout 비율, 가중치 초기화 방식에 따라 학습 결과가 어떻게 달라지는지 확인하는 것을 목표로 한다.
...
| 4 | Adam | True | True | 0.3 | Xavier | 98.41% | 537,354 |
...
### 3.4 Adam + BatchNorm True + Dropout True + ratio=0.3 + Xavier

He 초기화 대신 Xavier 초기화를 사용했다.

정확도는 `98.41%`로 He 초기화의 `98.35%`와 거의 차이가 없었다.

ReLU에는 이론적으로 He 초기화가 더 적합하지만, Adam과 BatchNorm을 함께 사용하면 초기화 차이가 어느 정도 보정되어 최종 정확도 차이가 작게 나타날 수 있다.
...
초기화 방식에 따른 차이는 매우 작았다.

Adam과 BatchNorm이 학습을 안정화하기 때문에 He와 Xavier의 차이가 최종 정확도에서는 크게 드러나지 않은 것으로 볼 수 있다.
...
BatchNorm과 Dropout은 Adam과 함께 사용할 때 성능을 소폭 개선했지만, Dropout 비율이나 He/Xavier 초기화 변화는 큰 차이를 만들지 않았다.

## 6. 레포트용 요약 문장

Adam 기반 실험에서는 모든 조합이 98% 이상의 정확도를 기록했다. BatchNorm과 Dropout을 사용하지 않은 기본 Adam 모델도 98.08%의 높은 정확도를 보였고, BatchNorm과 Dropout을 함께 적용했을 때 98.42%로 가장 좋은 결과를 얻었다. Dropout 비율을 0.5에서 0.3으로 낮췄을 때 정확도 차이는 거의 없었으며, He 초기화와 Xavier 초기화의 차이도 매우 작았다. 이를 통해 Adam optimizer가 SGD보다 빠르고 안정적으로 수렴하며, BatchNorm과 Dropout은 Adam과 함께 사용할 때 성능을 소폭 개선할 수 있음을 확인했다.
...
초기화 방식인 He와 Xavier의 차이는 두 optimizer 실험 모두에서 크지 않았다.
