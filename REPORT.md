...
| 관점 | 결론 |
| --- | --- |
| 최적화 방법 | Adam은 SGD보다 같은 에포크(epoch) 안에서 훨씬 빠르게 수렴했습니다. |
| BatchNorm | 제거해도 정확도 손실은 작았지만, 검증 손실은 기준 모델보다 높았습니다. |
| Dropout | 미사용 조건에서는 훈련 데이터에 더 빨리 맞춰졌지만, 검증 손실과 훈련-검증 차이가 커졌습니다. |
| 학습률 | 0.001 고정은 최종 정확도가 높았고, 감소 방식은 훈련-검증 차이가 더 작았습니다. |
| 기준 조건 | `Adam(학습률=0.001) + BatchNorm 사용 + Dropout 사용 + He 초기화` |

`adam_baseline`은 최종 검증 정확도와 검증 손실 기준에서 가장 균형 잡힌 결과를 보였습니다. 이번 실험의 핵심은 최고 정확도 하나를 고르는 것이 아니라, 설정을 바꿨을 때 학습 과정이 어떻게 달라지는지 확인하는 것이었습니다. 정확도만 보면 일부 설정은 비슷해 보였지만, 검증 손실과 훈련-검증 차이까지 함께 보면 기준 모델이 더 안정적이었습니다. 따라서 모델을 비교할 때는 정확도 하나보다 수렴 속도, 검증 손실, 훈련-검증 차이를 함께 보는 것이 적절했습니다.
